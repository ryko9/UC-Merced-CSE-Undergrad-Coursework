...
void MyViewer::build_scene()
{
	...
	p = new SnPrimitive(GsPrimitive::Cylinder, .005F, .012F, .3F); //second hand
	p->prim().material.diffuse = GsColor::red;
...
void MyViewer::run_animation()
{
	...
	GsMat ms = manip1->mat();
	SnManipulator* manip2 = rootg()->get<SnManipulator>(2);
	GsMat mm = manip2->mat();

	float radSec = (float) GS_PI / 30;  //incrementing by 6 degrees , pi / 30 = 6, to increment by 6
	float radMin = (float) GS_PI / 1800; // one-tenth, pi /1800 = 1/10, to increment by 1/10
	...
	do // run for a while:
	{
		...
		manip1->initial_mat(ms);
		manip2->initial_mat(mm);
		render();
		ws_check();
	} while (isRotate); //while rotating
	_animating = false;
...
int MyViewer::handle_keyboard ( const GsEvent &e )
{
	...
	{	case GsEvent::KeyEsc : gs_exit(); return 1;
		...
		case  ' ': isRotate = !(isRotate); //when space is pressed, set rotate to true, starting animation. !  negates whatever state rotation is, allowing for pauses
			run_animation();
			return 1;

		case GsEvent::KeyEnter: min = sec = 0.0f;
			isRotate = false;
			return 1;
