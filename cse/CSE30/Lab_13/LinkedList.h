...
void LinkedList<T>::insertAtBack(T valueToInsert)
{
	...
	else //if list already has values
	{
		...
		Node<T>* increment = new Node<T>(); //used to increment through list
		...
		Node<T>* temp = new Node<T>(); //temp points to newly allocated node that will be added to list
...
bool LinkedList<T>::removeFromBack()
{
	...
	{
		Node<T>* increment = new Node<T>();//node to traverse through list
		increment = first;

		Node<T>* previous = new Node<T>(); //the node BEFORE increment.
...
{
	Node<T>* increment = new Node<T>(); //pointer to go through each node
	increment = first; //points to start of list
        ...
        {				//since last->next = NULL, loop would stop.
                ...
                Node<T>* nextNode = increment->next; //extra pointer to store address of the next node before we free memory at increment
