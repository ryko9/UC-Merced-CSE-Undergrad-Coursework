...
void LinkedList_char::insertAtBack(char valueToInsert)
{
	...
	{
		//have to traverse through LinkedList_char to go from current head to curent tail
		...
		while(increment->next != NULL)
		{
			increment = increment->next;
					      ...
					      we have reached the end of the LinkedList_char*/
		}
...
bool LinkedList_char::removeFromBack()
{
	...
	else
	{
		...
		delete increment; //increment is pointing to the node that we removed from the LinkedList_char. we can now clear memory it is pointing to
...
bool LinkedList_char::removeFromFront()
{
	...
	{
		Node* newFirst = new Node();
		newFirst = first->next; //new node pochars to the node after first
