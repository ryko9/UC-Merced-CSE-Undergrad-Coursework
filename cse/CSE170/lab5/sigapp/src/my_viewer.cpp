...
int texture = 0; //to switch between files for texture
...
void MyViewer::build_scene()
{
	...
	GsModel& torus = *s->model(); //access the GsModel
	...
	if(texture == 0)
	{
		g.dmap->fname.set("../src/swirl.jpg");
	}
	else if(texture == 1)
	{
		g.dmap->fname.set("../src/wave.jpg");
	}
	else if(texture == 2)
	{
		g.dmap->fname.set("../src/cube.jpg");
	}

	
	//3. Make sure the number of materials matches num of groups
	torus.M.push().init();
	torus.M.top() = GsMaterial();

	//4. Now you can add texture coordinates 

	int nv = torus.V.size();
	torus.T.size(nv);

	int j = 0;
	for (theta = 0.0; theta < twoPi; theta += step)//nested for loop iterating through theta and phi
	{
		for (phi = 0.0; phi < twoPi; phi += step) //Use corresponding push order from generating triangles to determine coordinates
		{
			torus.T[j].set(theta / twoPi, phi / twoPi); //A
			torus.T[j + 1].set(theta / twoPi, (phi + step) / twoPi); //C
			torus.T[j + 2].set((theta + step) / twoPi, (phi + step) / twoPi); //D
			torus.T[j + 3].set(theta / twoPi, phi / (2 * pi)); //A
			torus.T[j + 4].set((theta + step) / twoPi, (phi + step) / twoPi); //D
			torus.T[j + 5].set((theta + step) / twoPi, phi / twoPi); //B
...
int MyViewer::handle_keyboard ( const GsEvent &e )
{
	...
	case GsEvent::KeySpace:
	{
		if (texture == 0)
		{
			texture++; //after pressing space once, switch to next texture
		}
		else if (texture == 1)
		{
			texture++; //switch to next texture by incrementing int texture
		}
		else if (texture == 2)
		{
			texture = 0; //if texture reaches 2, the last texture, return to 0
		}
		rootg()->remove_all();
		build_scene();
		render();
