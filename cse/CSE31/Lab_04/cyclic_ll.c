...
int has_cycle(node *head)
{
	...
	if(head == NULL)
	{
		return 0; //false if head points to NULL
	}
	...
	while(head != NULL)
	{
		A = A->next; //increment by 1.

		if(B->next != NULL)
		{
			B = B->next->next; //incrmeent by 2, IF there is space.
		}
		else
		{
			return 0;
		...
		if(A == NULL || B == NULL) //if either point to null, there is no loop
		{
			return 0;
		}
		if(A == B && A->next == B->next)
		{
			return 1;  //if algorithm's conditions are met, return true.
		}
	}
