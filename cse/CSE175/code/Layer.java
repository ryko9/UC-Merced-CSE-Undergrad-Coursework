...
public class Layer {
	...
	public void computeOutputDelta() {
		...
		Vector errorDiff; //Variable to store difference between vector and act
		Vector f2; //Derivative of activationFunction
		double a; //for storing iterations of error[i];
		double b; //for storing iterations of net[i];

		errorDiff = targ.difference(act); //Error = Target[i] - act[i]
		f2 = net.derivative(min, max); //Derivative of net[i]


		for(int i = 0; i < n; i++) //for loop to iterate through i values and calculate delta[i]
		{
			//delta = error * derivative of activationFunction

			a = errorDiff.get(i); //error[i] for this iteration
			b = f2.get(i); //derivative of net[i] for this iteration

			delta.set(i, a * b); //Calculate delta with set(i, a * b) for this layer
	...
	public void computeHiddenDelta() {
		...
		errorDiff = new Vector(n, 0.0);	//delta in n-dimension
		f2 = net.derivative(min, max); //Derivative of activationFunction(net[i])

		for(Projection i : outputs)
		{
			Matrix index = (i.W.transpose()); //store output in a matrix, Index
			Vector product = index.product(i.output.delta); //backpropogate through each layer and change weights

			errorDiff = errorDiff.sum(product); //Sum of (delta[k] * weight[i][k] )
		}
